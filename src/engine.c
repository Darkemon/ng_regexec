...
static int matcher(struct re_guts *g, char *string, size_t nmatch, regmatch_t pmatch[], int eflags)
{
  ...
  for (;;) 
  {
    ...
    if (m->pmatch == NULL)
    {
      m->pmatch = (regmatch_t *) malloc((m->g->nsub + 1) * sizeof(regmatch_t), M_REGEX, M_NOWAIT);
    }
    if (m->pmatch == NULL) 
    {
      STATETEARDOWN(m);
    ...
    else 
    {
      ...
      {
        m->lastpos = (char **) malloc((g->nplus + 1) * sizeof(char *), M_REGEX, M_NOWAIT);
      }
      if (g->nplus > 0 && m->lastpos == NULL) 
      {
        free(m->pmatch, M_REGEX);
        STATETEARDOWN(m);
        return REG_ESPACE;
      }
  ...
  if (m->pmatch != NULL) free(m->pmatch, M_REGEX);
  if (m->lastpos != NULL) free(m->lastpos, M_REGEX);
  STATETEARDOWN(m);
  return 0;
}
