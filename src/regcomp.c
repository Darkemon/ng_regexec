...
MALLOC_DEFINE(M_REGEX, "kernel_regex", "Regexp for using into kernel space");
...
int regcomp(regex_t *preg, char *pattern, int cflags)
{
  ...
  g = (struct re_guts *) malloc(sizeof(struct re_guts) + (NC - 1) * sizeof(cat_t), M_REGEX, M_NOWAIT);
  if (g == NULL) return REG_ESPACE;
  p->ssize = len / 2* 3 +  1; // ugh
  p->strip = (sop *) malloc(p->ssize * sizeof(sop), M_REGEX, M_NOWAIT);
  p->slen = 0;
  if (p->strip == NULL) 
  {
    free(g, M_REGEX);
...
static cset *allocset(struct parse *p)
{
  ...
  {  
    ...
    if (p->g->sets == NULL)
      p->g->sets = (cset *) malloc(nc * sizeof(cset), M_REGEX, M_NOWAIT);
    else
      p->g->sets = (cset *) realloc(p->g->sets, nc * sizeof(cset), M_REGEX, M_NOWAIT);
    if (p->g->setbits == NULL)
      p->g->setbits = (uch *) malloc(nbytes, M_REGEX, M_NOWAIT);
    else 
    {
      p->g->setbits = (uch *) realloc(p->g->setbits, nbytes, M_REGEX, M_NOWAIT);
...
static void mcadd(struct parse *p, cset *cs, char *cp)
{
  ...
  if (cs->multis == NULL)
    cs->multis = malloc(cs->smultis, M_REGEX, M_NOWAIT);
  else
    cs->multis = realloc(cs->multis, cs->smultis, M_REGEX, M_NOWAIT);
  if (cs->multis == NULL) 
  {
    SETERROR(REG_ESPACE);
    return;
...
static void enlarge(struct parse *p, sopno size)
{
  ...
  sp = (sop *) realloc(p->strip, size * sizeof(sop), M_REGEX, M_NOWAIT);
  if (sp == NULL) 
  {
    SETERROR(REG_ESPACE);
    return;
...
{
  g->nstates = p->slen;
  g->strip = (sop *) realloc(p->strip, p->slen * sizeof(sop), M_REGEX, M_NOWAIT);
  if (g->strip == NULL) 
  {
    SETERROR(REG_ESPACE);
    g->strip = p->strip;
...
static void findmust(struct parse *p, struct re_guts *g)
{
  ...
  g->must = malloc((size_t) g->mlen + 1, M_REGEX, M_NOWAIT);
  if (g->must == NULL) 
  {    
    // argh; just forget it
    g->mlen = 0;
